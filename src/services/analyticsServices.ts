...
export const getAnalyticServiceCount = async (req: Request) => {
    ...
    const { query, status = "Pending", from, to } =
    ...
    if (query === "priority") {
        const [high, medium, low] =
            await countFieldsHelper('priorityLevel', priorityValues, dateRange);
        return {
            countByPriority:
                { high, medium, low }
        };
    ...
    if (query === "category") {
        const [complain, maintenance, other] =
            await countFieldsHelper('category', categoryValues, dateRange);
        return {
            countByCategory:
    ...
    if (query === "status") {
        if (status === "Pending") {
            const [all, high, medium, low] =
                await countFieldsHelper('status', priorityValues, dateRange, status);
            return {
                countByStatus:
                    { [status.toLowerCase()]: { all, high, medium, low } }
            };
        };
        if (status === "Ongoing") {
            const [all, high, medium, low] =
